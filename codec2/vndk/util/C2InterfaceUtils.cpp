...
C2SupportedRange<T>::C2SupportedRange(const C2FieldSupportedValues &values) {
    if (values.type == C2FieldSupportedValues::RANGE) {
        _mMin = values.range.min.ref<T>();
        _mMax = values.range.max.ref<T>();
        _mStep = values.range.step.ref<T>();
        _mNum = values.range.num.ref<T>();
        _mDenom = values.range.denom.ref<T>();
    } else {
        _mMin = MAX_VALUE;
        _mMax = MIN_VALUE;
        _mStep = MIN_STEP;
